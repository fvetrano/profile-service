...
import it.eng.tim.profilo.integration.proxy.ProxyTemplate;
...
public class ProxyTemplateTest {
    ...
    @Test
    public void validateKo() throws Exception {
        assertFalse(template.validate(null));
        assertFalse(template.validate(new ResponseEntity<>(HttpStatus.BAD_GATEWAY)));
    }

    @Test
    public void validateOk() throws Exception {
        assertTrue(template.validate(new ResponseEntity<>(HttpStatus.OK)));
    }

    @Test(expected = SubsystemException.class)
    public void getBodyInvalidResponse() throws Exception {
        assertFalse(template.getBody(null));
    }
